...
class FA_encoder(nn.Module):
    def __init__(self, num_resnet_layers,dropout_rate: float = .09):
        ...
        if self.num_resnet_layers == 50:
            ...
            self.inplanes = 2048
        ########  Thermal ENCODER  ########
        self.encoder_gas_conv1 = nn.Conv2d(
            1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.encoder_gas_conv1.weight.data = torch.unsqueeze(
            torch.mean(resnet_raw_model1.conv1.weight.data, dim=1), dim=1)
        self.encode_dropout = MCDropout(dropout_rate)

        self.encoder_gas_bn1 = resnet_raw_model1.bn1
        self.encoder_gas_relu = resnet_raw_model1.relu
        self.encoder_gas_maxpool = resnet_raw_model1.maxpool
        self.encoder_gas_layer1 = resnet_raw_model1.layer1
        self.encoder_gas_layer2 = resnet_raw_model1.layer2
        self.encoder_gas_layer3 = resnet_raw_model1.layer3
        self.encoder_gas_layer4 = resnet_raw_model1.layer4
        ...
        self.encoder_bg_conv1 = nn.Conv2d(
            1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.encoder_bg_conv1.weight.data = torch.unsqueeze(
            torch.mean(resnet_raw_model2.conv1.weight.data, dim=1), dim=1)
        self.encoder_bg_bn1 = resnet_raw_model2.bn1

        # 增加MCD机制
        self.encode_dropout = MCDropout(dropout_rate)
        self.encoder_bg_relu = resnet_raw_model2.relu
        self.encoder_bg_maxpool = resnet_raw_model2.maxpool
        self.encoder_bg_layer1 = resnet_raw_model2.layer1
        self.encoder_bg_layer2 = resnet_raw_model2.layer2
        self.encoder_bg_layer3 = resnet_raw_model2.layer3
        self.encoder_bg_layer4 = resnet_raw_model2.layer4

    def forward(self, bg, gas):
        ######################################################################
        # layer0
        ######################################################################
        bg = self.encoder_bg_conv1(bg)
     
        bg = self.encoder_bg_bn1(bg)

        bg = self.encoder_bg_relu(bg)

        bg = self.encoder_bg_maxpool(bg)

        gas = self.encoder_gas_conv1(gas)

        gas = self.encoder_gas_bn1(gas)

        gas = self.encoder_gas_relu(gas)

        gas = self.encoder_gas_maxpool(gas)

        ######################################################################

        fuse0 = self.ae0(gas,bg)

        ######################################################################
        # layer1
        ######################################################################
        bg1 = self.encoder_bg_layer1(bg)
        gas1 = self.encoder_gas_layer1(gas)
        ...
        # layer2
        ######################################################################
        bg2 = self.encoder_bg_layer2(bg1)
        gas2 = self.encoder_gas_layer2(gas1)
        ...
        # layer3
        ######################################################################
        bg3 = self.encoder_bg_layer3(bg2)
        gas3 = self.encoder_gas_layer3(gas2)
        ...
        # layer4
        ######################################################################
        bg4 = self.encoder_bg_layer4(bg3)
        gas4 = self.encoder_gas_layer4(gas3)
...
class Cascaded_decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        ##############################################################################
        x, x1, x5 = self.HA(attention_gate, bg, bg1, bg2)
        x0_2 = self.rfb0_2(x)
        ux0_2 = x0_2
        x1_2 = self.rfb1_2(x1)
        ux1_2 = x1_2
...
def model_stat():
    model = GasSegNet(2).cuda(0)
    input = [torch.randn(1,1,512,640).cuda(),torch.randn(1,1,512,640).cuda()]
    from thop import profile
