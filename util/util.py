...
def show_video(predictions, origin):
    palette = get_palette()
    for (i, pred) in enumerate(predictions):
        pred = predictions[i].cpu().numpy()
        img = np.zeros((pred.shape[0], pred.shape[1], 3), dtype=np.uint8)
...
def save_video(predictions, video, origin):
    palette = get_palette()
    for (i, pred) in enumerate(predictions):
        pred = predictions[i].cpu().numpy()
        img = np.zeros((pred.shape[0], pred.shape[1], 3), dtype=np.uint8)
...
def visualize_gt2(label_path, image_name, predictions, weight_name):
    label_path = label_path
    palette = get_palette()
    for (i, pred) in enumerate(predictions):
        pred = predictions[i].cpu().numpy()
        gt = cv2.imread(os.path.join(label_path,image_name,"gt.png"))
        # gt = gt * 255
        # Convert the expanded image to uint8 data type
        gt = gt.astype(np.uint8)
        img = np.zeros((pred.shape[0], pred.shape[1], 3), dtype=np.uint8)
...
def compute_results2(conf_total, beta=1.0):
    ...
    # Acc_per_class = np.zeros(n_class)
    ...
    for cid in range(start_index, n_class):
        # TP = conf_total[cid, cid]  # 对角线元素
        # FP = np.sum(conf_total[:, cid]) - TP  # 第 i 列的总和减去 TP
        # FN = np.sum(conf_total[cid, :]) - TP  # 第 i 行的总和减去 TP
        # TN = np.sum(conf_total) - (TP + FP + FN)  # 总和减去 TP, FP, FN
